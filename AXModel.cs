...
namespace ACUtils.AXRepository
{
    public abstract class AXModel<T> : DBModel<T> where T : AXModel<T>, new()
    {
        ...
        protected override void setValue(string key, object value, Type colType = null)
        {
            ...
            if (sourceType == typeof(Abletech.WebApi.Client.Arxivar.Model.UserProfileDTO))
            {
                if (targetType == typeof(string))
                {
                    value = ((Abletech.WebApi.Client.Arxivar.Model.UserProfileDTO)value).Description;
                }
                if (targetType == typeof(int) || targetType == typeof(int?))
                {
                    value = ((Abletech.WebApi.Client.Arxivar.Model.UserProfileDTO)value).AddressBookId;
                }
            }

            if (sourceType == typeof(Abletech.WebApi.Client.ArxivarManagement.Model.UserProfileDTO))
            {
                if (targetType == typeof(string))
                {
                    value = ((Abletech.WebApi.Client.ArxivarManagement.Model.UserProfileDTO)value).Description;
                }
                if (targetType == typeof(int) || targetType == typeof(int?))
                {
                    value = ((Abletech.WebApi.Client.ArxivarManagement.Model.UserProfileDTO)value).AddressBookId;
                }
            }
            if (sourceType == typeof(List<Abletech.WebApi.Client.Arxivar.Model.UserProfileDTO>))
            {
                var source = (List<Abletech.WebApi.Client.Arxivar.Model.UserProfileDTO>)value;
        ...
        public static T Idrate(Abletech.WebApi.Client.Arxivar.Model.EditProfileSchemaDTO model)
        {
            var obj = new T();
            ...
            var mittente = model.Fields.GetField<Abletech.WebApi.Client.Arxivar.Model.ToFieldDTO>("TO");
            obj.SetPropertyIfExists(AxToExternalIdFieldAttribute.AX_KEY, mittente.Value?.Select(m => m.ExternalId));

            var destinatario = model.Fields.GetField<Abletech.WebApi.Client.Arxivar.Model.FromFieldDTO>("FROM");
            obj.SetPropertyIfExists(AxFromExternalIdFieldAttribute.AX_KEY, destinatario.Value?.ExternalId);
            return obj;
        }

        public static T Idrate(Abletech.WebApi.Client.Arxivar.Model.RowSearchResult searchresult)
        {
            var obj = new T();
        ...
        public static List<T> Idrate(List<Abletech.WebApi.Client.Arxivar.Model.RowSearchResult> results)
        {
